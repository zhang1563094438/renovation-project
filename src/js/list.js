$(function() {
    ...
    async function getCateOne() {
        // 1-2. 发送请求获取
        const cat_one_list = await $.get('/zhang/getOne.php', null, null, 'json')
        ...
        $('.onebox > .right').html(str)
    }

    // 1-2. 请求二级分类列表
    async function getCateTwo() {
        // 1. 请求二级分类列表数据
        const cate_two_list = await $.get('/zhang/getTwo.php', { cat_one: list_info.cat_one }, null, 'json')
    ...
    async function getCateThree() {
        // 1. 请求二级分类列表数据
        const cate_three_list = await $.get('/zhang/getThree.php', { cat_one: list_info.cat_one, cat_two: list_info.cat_two }, null, 'json')
    ...
    async function getTotalPage() {
        // 2-1. 请求分页数据
        const totalInfo = await $.get('/zhang/getTotalPage.php', list_info, null, 'json')
    ...
    async function getGoodsList() {
        // 3-1. 请求商品列表
        const goodsList = await $.get('/zhang/getGoodsList.php', list_info, null, 'json')
        ...
        goodsList.list.forEach(item => {
            str += `
          <li class="thumbnail">
            <img src="${ item.goods_big_logo }" data-id="${ item.goods_id }" alt="...">
            <div class="god" ><img src="${ item.goods_small_logo }"><img src="../images/logo.png"></div>
            <div class="caption">
              <h3 data-id="${ item.goods_id }">${ item.goods_name }</h3>
              <p class="price">￥
                <span class="text-danger" data-id="${ item.goods_id }" >${ item.goods_price }</span>
    ...
    $('.onebox').on('click', 'span', function() {
        ...
        getTotalPage()
        getGoodsList()
        $('.threebox .right').html('<span data-type="all" class="active">全部</span>')

        // 4-5. 判断 type 是否为 all 信息
        if (type === 'all') {
            // 让二级列表回到 全部 状态
            // 改变结构
            $('.twobox .right').html('<span data-type="all" class="active">全部</span>')
        } else {
            // 根据一级分类 请求 二级分类列表 渲染
            getCateTwo()
        }
    ...
    // 6. 三级分类的点击事件
    // 6-1. 事件委托
    $('.threebox').on('click', 'span', function() {
        const type = $(this).data('type')
